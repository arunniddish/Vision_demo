...
%%% tracked seperately. 
%%% This code runs the complete video. It also takes in the account of
%%% Y-axis been reversed. 
%%% Also converts Pixels --> cm
...
baseFileName = 'Inchworm_2_full.mp4';  % Video to track
...
for k = 1:numberOfFrames
	...
	if numberOfRegions == 7
		...
		for r = 1 : numberOfRegions
            ...
            thisCentroid = [thisCentroid(1)-x_correction y_correction-thisCentroid(2)];
            
            % Nearest neighbour
            if(count==1 && r==1)
%                 thisCentroid = [thisCentroid(1)-x_correction thisCentroid(1)-y_correction]; 
...
fulltextfilename = fullfile(folder,filenametowrite);
fileid = fopen(fulltextfilename,'a+');
plot(centroid1(55,1),centroid1(55,2),'y+','MarkerSize',4,'LineWidth',1);

filenametowrite = 'cent7.txt';
fulltextfilename = fullfile(folder,filenametowrite);
fileid = fopen(fulltextfilename,'a+');
plot(centroid7(55,1),centroid7(55,2),'y+','MarkerSize',4,'LineWidth',1);
 

% Draw a straight line between 2 points
A = [centroid1(55,1) centroid7(55,1)]; 
B = [centroid1(55,2) centroid7(55,2)]; 
line(A,B,'LineWidth',2)

r_l_cord = [centroid1(55,:);centroid7(55,:)];
r_l = pdist(r_l_cord,'euclidean');
