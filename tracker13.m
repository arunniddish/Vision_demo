...
% Measured length of the Inch worm
worm_len = 89.8;
...
figure(3)
imshow(thisFrame);
title(' Natural Length', 'FontSize', fontSize);
hold on
...
fulltextfilename = fullfile(folder,filenametowrite);
fileid = fopen(fulltextfilename,'a+');
plot(centroid1(1,1),centroid1(1,2),'y+','MarkerSize',4,'LineWidth',1);

filenametowrite = 'cent7.txt';
fulltextfilename = fullfile(folder,filenametowrite);
fileid = fopen(fulltextfilename,'a+');
plot(centroid7(1,1),centroid7(1,2),'y+','MarkerSize',4,'LineWidth',1);
 

% Draw a straight line between 2 points
A = [centroid1(1,1) centroid7(1,1)]; 
B = [centroid1(1,2) centroid7(1,2)]; 
line(A,B,'LineWidth',2)

length_cord = [centroid1(1,:);centroid7(1,:)];
n_len = pdist(length_cord,'euclidean');
pix2mm = worm_len/n_len;
